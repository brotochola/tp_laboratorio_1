...
int al_set(ArrayList* pList, int index,void* pElement)
{
     ...
     // printf("DEBUG! %d-> %s %s: %f\n",(pList->pElements+index), ((movie*)pElement)->name, ((movie*)pElement)->lastName, ((movie*)pElement)->salary);
...
int al_remove(ArrayList* pList,int index)
{
    ...
    /*  movie* contenido;
      contenido = (movie*)get(pList, index);*/
...
ArrayList* al_clone(ArrayList* pList)
{
    ...
    returnAux= al_subList(pList, 0, al_len(pList));
...
{
   // printf("DEBUG! [%p] %s %s: %f\n",pElement,  ((movie*)pElement)->name, ((movie*)pElement)->lastName, ((movie*)pElement)->salary);
    int returnAux = -1;
    if(pList==NULL || pElement==NULL) return -1;
    int i;
...
int al_containsAll(ArrayList* pList,ArrayList* pList2)
{
    ...
    for(i=0; i< pList->size; i++){
      //  printf("pList [%d] id=%d \n",i,  ((movie*)*(pList->pElements+i))->id);
      //  printf("pList2 [%d] id=%d \n",i,  ((movie*)*(pList2->pElements+i))->id);
  ...
  movie* empAux;
    ...
    for(i=0; i<pList->size; i++){
        for(j=i+1; j<pList->size; j++){
           peli1 = al_get(pList, i);
            peli2 = al_get(pList, j);
          //  printf("peli i %s\n", peli1->titulo);
         //   printf("peli j %s\n", peli2->titulo);
         //   printf("comparar: %d\n", pFunc(peli1, peli2));

            if(pFunc(peli1, peli2)>0 && order==1){
                    //pFunc es como strcomp.
            ...
            } else    if(pFunc(peli1, peli2)<0 && order==0){
                empAux=*(pList->pElements+j);
                *(pList->pElements+j) = *(pList->pElements+i);
                *(pList->pElements+i) = empAux;
